...
解释：在范围 [120, 130] 内：
120：中间数位 2 是峰，波动值 = 1。121：中间数位 2 是峰，波动值 = 1。130：中间数位 3 是峰，波动值 = 1。
范围内所有其他数字的波动值均为 0。因此，总波动值为 1 + 1 + 1 = 3。

示例 2：输入： num1 = 198, num2 = 202  输出： 3
解释：在范围 [198, 202] 内：
198：中间数位 9 是峰，波动值 = 1。201：中间数位 0 是谷，波动值 = 1。202：中间数位 0 是谷，波动值 = 1。
范围内所有其他数字的波动值均为 0。因此，总波动值为 1 + 1 + 1 = 3。
...
题解路径：. / leetcode_daily_stories / 26-06-05.md
...
class Solution:
    def totalWaviness(self, num1: int, num2: int) -> int:
        ...
        @cache
        def dfs(i, last_cmp, last_digit, limit_lo, limit_hi):
            if i == n:
                return 0, 1
            lo = low_s[i-diff_lh] if limit_lo and i>=diff_lh else 0
            hi = high_s[i] if limit_hi else 9

            waviness = num_cnt = 0
            is_num = not limit_lo or i>diff_lh
            for d in range(lo, hi+1):
                c = (d>last_digit) - (d<last_digit) if is_num else 0
                sub_waviness, sub_num_cnt = dfs(i+1, c, d, limit_lo and d==lo, limit_hi and d==hi)
                waviness += sub_waviness
                num_cnt += sub_num_cnt
                if c * last_cmp < 0:
                    waviness += sub_num_cnt
            return waviness, num_cnt
