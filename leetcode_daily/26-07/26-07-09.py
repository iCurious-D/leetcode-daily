...
示例 1：输入: n = 2, nums = [1,3], maxDiff = 1, queries = [[0,0],[0,1]];    输出: [true,false]
解释:查询 [0,0]：节点 0 有一条到自己的显然路径。
查询 [0,1]：节点 0 和节点 1 之间没有边，因为 |nums[0] - nums[1]| = |1 - 3| = 2，大于 maxDiff。
因此，在处理完所有查询后，最终答案为 [true, false]。

示例 2：输入: n = 4, nums = [2,5,6,8], maxDiff = 2, queries = [[0,1],[0,2],[1,3],[2,3]]
...
def pathExistenceQueries(n: int, nums: List[int], maxDiff: int, queries: List[List[int]]) -> List[bool]:
    ...
    # rights.append(n-1)
    # return [bisect_left(rights, l) == bisect_left(rights, r) for l, r in queries]
...
if __name__ == '__main__':
    print(pathExistenceQueries(n = 2, nums = [1,3], maxDiff = 1, queries = [[0,0],[0,1]]))
    print(pathExistenceQueries(n = 4, nums = [2,5,6,8], maxDiff = 2, queries = [[0,1],[0,2],[1,3],[2,3]]))
    print(pathExistenceQueries(n = 3, nums = [1,2,3], maxDiff = 2, queries = [[0,2],[2,1],[0,1]]))
