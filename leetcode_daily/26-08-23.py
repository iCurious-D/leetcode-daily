...
解释：num 中没有 '?' ，没法进行任何操作。前一半的和等于后一半的和：5 + 0 = 2 + 3 。
示例 2：输入：num = "25??";   输出：true
解释：Alice 可以将两个 '?' 中的一个替换为 '9' ，Bob 无论如何都无法使前一半的和等于后一半的和。
示例 3：输入：num = "?3295???";   输出：false
解释：Bob 总是能赢。一种可能的结果是：- Alice 将第一个 '?' 用 '9' 替换。num = "93295???" 。
...
def sumGame(num: str) -> bool:
    ...
    #
    # half = len(num) // 2
    # left_q, left_sum = calc(num[half:])
    # right_q, right_sum = calc(num[:half])
    # return (left_q + right_q) % 2 > 0 or right_sum - left_sum != (left_q - right_q) // 2 * 9
    ...
    return questions_gap % 2 != 0 or left_sum - right_sum + questions_gap // 2 * 9 == 0
