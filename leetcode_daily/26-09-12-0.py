...
示例 2：输入：startTime = [1,2,3,4,6], endTime = [3,5,10,6,9], profit = [20,20,100,70,60];    输出：150
解释：我们选择第 1，4，5 份工作。 共获得报酬 150 = 20 + 70 + 60。
...
def jobScheduling(startTime: List[int], endTime: List[int], profit: List[int]) -> int:
    ...
    for i, (s, e, p) in enumerate(jobs):
        f[i + 1] = max(f[i], p + f[bisect_right(jobs, s, key=lambda v: v[1])])
    return f[-1]
